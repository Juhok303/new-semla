...
class AdaLN(torch.nn.Module):
    ...
    def __init__(self, d_edge, d_model=None):
        super().__init__()

        d_model = d_edge if d_model is None else d_model

        self.w_in = torch.nn.Linear(d_edge, d_edge, bias=True)
        self.w_out = torch.nn.Linear(d_edge, d_edge, bias=True)
        self.w_edge = torch.nn.Linear(d_edge, d_edge, bias=True)

        self.a = torch.nn.Linear(3 * d_edge, 1, bias=True)
        
        self.activate = torch.nn.SiLU()
        self.Linear = torch.nn.Linear(d_edge, 6 * d_model, bias=True)
    ...
    def forward(self, edge_feats):
        ...
        Returns:
            torch.Tensor: Edge messages tensor, shape [batch_size, n_nodes, d_model] * 6
        """
        
        batch_size, n_nodes, _, _ = edge_feats.shape

        # attn: shape [B, N, d_edge]
        aggr_edge = torch.diagonal(edge_feats, offset=0, dim1=1, dim2=2).transpose(-1, -2)

        # w_in, w_out, w_edge: shape [B, N, N, d_edge]
        
        w_in = self.w_in(aggr_edge).unsqueeze(2).expand(batch_size, n_nodes, n_nodes, -1)
        w_out = self.w_out(aggr_edge).unsqueeze(1).expand(batch_size, n_nodes, n_nodes, -1)
        w_edge = self.w_edge(edge_feats)

        # attn, weight: shape [B, N, N, 1]
        attn = self.activate(self.a(torch.concat([w_in, w_out, w_edge], dim=-1)))
        weight = torch.softmax(attn, dim=-2)
        
        aggr_edge = aggr_edge + (edge_feats * weight).sum(2)
        
        messages = self.activate(aggr_edge)
        gate_attn, shift_attn, scale_attn, gate_mlp, shift_mlp, scale_mlp = self.Linear(messages).chunk(6, dim=-1)
        
        return gate_attn, shift_attn, scale_attn, gate_mlp, shift_mlp, scale_mlp
...
class NodeAttention(torch.nn.Module):
    def __init__(self, d_model, n_attn_heads, d_attn=None, adaln=False):
        super().__init__()
    ...
    def forward(self, node_feats, messages, adj_matrix, gate=None, shift=None, scale=None):
        ...
        Args:
            ...
            gate, shift, scale (torch.Tensor): Adaptive LayerNorm tensor, shape [batch_size, n_nodes, d_model]

        Returns:
            torch.Tensor: Accumulated node features, shape [batch_size, n_nodes, d_model]
        """

        if (self.adaln) and ((gate is not None) or (shift is not None) or (scale is not None)):
            raise ValueError("Gate, shift, scale were provided but the model was initialised with AdaLN as None.")

        if (not self.adaln) and ((gate is None) or (shift is None) or (scale is None)):
        ...
        if self.adaln :
            node_feats = node_feats * (1 + scale) + shift
...
class NodeFeedForward(torch.nn.Module):
    def __init__(self, d_model, n_coord_sets, d_ff=None, proj_sets=None, coord_norm="length", adaln=False):
        super().__init__()
    ...
    def forward(self, coord_sets, node_feats, node_mask, gate=None, shift=None, scale=None):
        """Pass data through the layer
        ...
        """
        if (self.adaln is False) and ((gate is not None) or (shift is not None) or (scale is not None)):
            raise ValueError("Gate, shift, scale were provided but the model was initialised with AdaLN as None.")

        if (self.adaln is True) and ((gate is None) or (shift is None) or (scale is None)):
            raise ValueError("The model was initialised with AdaLN but no gate, shift, scale were provided to forward fn.")
        ...
        if self.adaln:
            out_node_feats = gate * self.invariant_mlp(coord_sets, node_feats * (1 + scale) + shift)
        else:
...
class EquiMessagePassingLayer(torch.nn.Module):
    def __init__(
        ...
        if d_edge_in is not None:
            ...
            if adaln:
                self.adaln = AdaLN(d_edge_in, d_model)
        ...
        self.node_attn = NodeAttention(d_model, n_attn_heads, d_attn=d_attn, adaln=adaln)
    ...
    def forward(self, coords, node_feats, adj_matrix, node_mask, edge_feats=None):
        ...
        if (self.adaln is not None) and (edge_feats is None):
            raise ValueError("The model was initialised with AdaLN but no edge feats were provided to forward.")

        gate_attn, shift_attn, scale_attn, gate_mlp, shift_mlp, scale_mlp = None, None, None, None, None, None
        if self.adaln is not None:
            gate_attn, shift_attn, scale_attn, gate_mlp, shift_mlp, scale_mlp = self.adaln(edge_feats) 
            
        coord_updates, node_updates = self.node_ff(coords, node_feats, node_mask, gate=gate_mlp, shift=shift_mlp, scale=scale_mlp)
        ...
        node_feats = node_feats + self.node_attn(node_feats, node_messages, adj_matrix, gate=gate_attn, shift=shift_attn, scale=scale_attn)
...
class SemlaGenerator(MolecularGenerator):
    ...
    def forward(
        ...
        if pred_edges is not None:
            ...
            for _ in 
            transformed_atom_feats = torch.concat([atom_feats for _ in range(4)], dim=0)
            transformed_adj_matrix = torch.concat([adj_matrix for _ in range(4)], dim=0)
            transformed_atom_mask = torch.concat([atom_mask for _ in range(4)], dim=0)
            transformed_edge_feats = torch.concat([edge_feats for _ in range(4)], dim=0)
            #geographic transform on coords
            #concat coords
            codes = self.dynamics.layers[0](
                coords, atom_feats, adj_matrix, atom_mask=atom_mask, edge_feats=edge_feats
            )
            """
            #codes = None
            return pred_coords, type_logits, edge_logits, charge_logits#, codes
